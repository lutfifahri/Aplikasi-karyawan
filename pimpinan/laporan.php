...
$pdf->Cell(25.5,0.7,"Laporan Hasil Penilaian Metode MOORA dan Weighted Product",0,10,'C');
$pdf->ln(1);
...
$pdf->Cell(3, 0.8, 'Nilai Moora', 1, 0, 'C');
$pdf->Cell(3, 0.8, 'Peringkat Moora', 1, 0, 'C');
$pdf->Cell(3, 0.8, 'Nilai wp', 1, 0, 'C');
$pdf->Cell(3, 0.8, 'Peringkat wp', 1, 1, 'C');
$pdf->SetFont('Times','',10);
$no=1;
$query=mysql_query("select * from tbl_pelamar order by kode_pelamar asc");
while($lihat=mysql_fetch_array($query)){
	$pdf->Cell(2, 0.8, $no , 1, 0, 'C');
	$pdf->Cell(7, 0.8, $lihat['nama'], 1, 0,'C');
	$pdf->Cell(5, 0.8, $lihat['alamat'], 1, 0,'C');
	$pdf->Cell(3, 0.8, number_format($lihat['nilai_moora'],3), 1, 0,'C');
	$pdf->Cell(3, 0.8, $lihat['rangking_moora'], 1, 0,'C');
	$pdf->Cell(3, 0.8, number_format($lihat['nilai_wp'],3), 1, 0,'C');
	$pdf->Cell(3, 0.8, $lihat['rangking_wp'], 1, 1,'C');
	$no++;
}
